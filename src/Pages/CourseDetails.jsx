...
export default function CourseDetails() {
      ...
      <div className="flex relative   gap-8 ">
        <div className=" mt-12 px-12 min-h-[260px] max-w-maxContentTab flex-col justify-center  lg:max-w-maxContent w-[60%]">
          <p className="text-sm mb-4 text-richblack-300">
            {`Home / Catalog / `}
            <span className="text-yellow-25  ">{course?.category[0]?.name}</span>
          </p>
  ...
  <Accordion allowMultipleExpanded>
    {course?.courseContent?.map((item,index) => (
        ...
        <AccordionItem key={index}  className=" " backgroundColor="red">
            <AccordionItemHeading className=" " >
                <AccordionItemButton color="white" >
                    {item.sectionName}
                    <span className="text-yellow-25 text-sm mx-12 " >{item.subSections.length +'-'}Lectures</span> 
                </AccordionItemButton>
            </AccordionItemHeading>
            <AccordionItemPanel>
                 {
                    item?.subSections.map((subSection, index)=>{
                        ...
                        <div className=" text-white flex flex-col">
                          ...
                          <ReactPlayer url={subSection.videoUrl} />
                          </div>
                        </AccordionItemPanel>
                    </AccordionItem>
                    })
        ...
        <div className="text-white flex flex-col w-[30%]  rounded-lg absolute right-[10%] top-0 bg-richblack-800">
          <img
            src={course?.thumbnail}
            alt="laoding"
            className="w-[100%] rounded-lg mt-14"
          />
  ...
  <defs>
    <clipPath id="clip0_11167_19135">
      <rect width="14" height="14" fill="white"/>
              ...
              <p className="text-caribbeangreen-50 ">Certificate of completion</p>
            </p>
          </div>
        </div>
      </div>
